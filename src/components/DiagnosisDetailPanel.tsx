...
function DetailSection({
  ...
  return (
    <details className={`rounded-[10px] border ${tone}`} open={defaultOpen}>
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2">
        ...
        {count !== undefined ? (
          <span className="rounded-[7px] bg-white/80 px-2 py-0.5 text-xs font-bold">
            {count}
          </span>
        ) : null}
      </summary>
...
function RuleList({
  ...
  if (items.length === 0) {
    return (
      <p className="rounded-[8px] bg-white/80 px-2.5 py-2 text-xs leading-5 text-slate-500">
        {emptyText}
      </p>
    );
  }
  ...
  return (
    <ul className="grid gap-1.5">
      {items.map((item) => (
        <li
          key={item.id}
          className="rounded-[8px] bg-white/80 px-2.5 py-2 text-xs leading-5 text-slate-800"
        >
  ...
  return (
    <ul className="grid gap-1.5 md:grid-cols-2">
      {items.map((item) => (
        <li
          key={item}
          className="rounded-[8px] bg-white/80 px-2.5 py-2 text-xs leading-5 text-slate-700"
        >
          {item}
        </li>
      ))}
...
export function DiagnosisDetailPanel({ score }: DiagnosisDetailPanelProps) {
  if (!score) {
    return (
      <aside className="flex h-full min-h-[360px] items-center justify-center rounded-[12px] border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_1px_3px_rgba(16,24,40,0.06)] xl:min-h-0">
  ...
  return (
    <aside className="flex h-full min-h-[620px] flex-col overflow-hidden rounded-[12px] border border-slate-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04),0_1px_3px_rgba(16,24,40,0.06)] xl:min-h-0">
      <div className="sticky top-0 z-10 shrink-0 border-b border-slate-100 bg-white px-4 py-3">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          <span
            className={`rounded-[7px] border px-2 py-1 text-[11px] font-bold ${tone.className}`}
          >
            {evidenceStatusLabels[score.evidenceStatus]}
          </span>
          <span
            className={`rounded-[7px] border px-2 py-1 text-[11px] font-bold ${urgencyTone[diagnosis.urgency]}`}
          >
            {urgencyLabels[diagnosis.urgency]}
          </span>
          <span className="rounded-[7px] border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] font-bold text-slate-600">
        ...
        <div className="mt-3 grid gap-2">
          <div className="rounded-[10px] border border-red-100 bg-red-50 px-3 py-2">
            ...
            <ul className="grid gap-1 text-xs leading-5 text-red-950">
              {(score.missingKeyData.length > 0
                ? score.missingKeyData.slice(0, 3)
                : ["현재 누락된 핵심 정보가 없습니다."]
              ).map((item) => (
          ...
          <div className="rounded-[10px] border border-blue-100 bg-blue-50 px-3 py-2">
            <p className="mb-1 text-xs font-extrabold text-blue-700">
              권장 다음 평가
            </p>
            <p className="line-clamp-2 text-xs leading-5 text-blue-950">
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto bg-[#f7f9fc] p-3">
        ...
        >
          ...
          <p className="mt-2 rounded-[8px] bg-white/80 px-2.5 py-2 text-xs leading-5">
        ...
        <div className="rounded-[10px] border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-900">
